(() => {
  ...
  function getRawTitle() {
    const h = document.querySelector('.timeline-content .card-title.card-header') || document.querySelector('.card-title.card-header');
  ...
  async function downloadSelected() {
    ...
    for (const idx of sorted) {
      ...
      const pdfLinks = Array.from(card.querySelectorAll('a[href*="document.send.php"][title$=".pdf"]'));
      const allLinks = Array.from(card.querySelectorAll('a[href*="document.send.php"]'));
      const onlyPdfAttachment = pdfLinks.length === 1 && allLinks.length === 1;

      if (onlyPdfAttachment) {
        ...
        try {
          ...
          console.error('Erro ao importar PDF externo', link.href, e);
        }
      } else {
  ...
  // 🔥 Inicialização automática no clique da imagem de extensão
  document.querySelectorAll('.card-body img[src*="/pics/icones/"]').forEach(img => {
  ...
  // Botão "Exportar PDF" (ainda visível)
